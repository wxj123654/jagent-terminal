 ...
 * acpAgents；T3+.1 实装，占位卡下岗）。R8 对齐原型：简介行 + le-card 列表 +
 * le-add「新增 ACP agent」。
 *
 * 结构同 Presets 分区（列表 CRUD），但无 builtin/modified 概念——默认 2 项
 * （codex --acp / claude-code-acp）也只是可删改的示例（preset 的「内置」是
 * 工厂数据，agent 的「默认」是建议）。行收起态：chevron + label + mono
 * 命令摘要 + ACP 徽章 + 删除钮；展开态三字段编辑器（label / command / args）。
 *
...
import { TextInput, COLORS, FONT } from '@jagent/ui'
...
export function AcpAgentsSection({
  ...
  return (
    ...
    <div style={{ minWidth: 0 }}>
      ...
      >
        外部 agent 进程，经 ACP 协议接入（JSON-RPC over stdio）。凭证走 shell 环境——此处不出现 API
        key 输入框。
      </text>
      ...
      <ListEditorAdd
        ...
        label="新增 ACP agent"
        onAdd={() => setExpandedId(settings.addAcpAgent({ label: 'New Agent' }))}
      />
    </div>
  )
}
...
function AgentCard({
  ...
  return (
    <ListEditorCard
      ...
      expanded={expanded}
      onToggle={onToggle}
      badges={<HeadBadge text="ACP" testId={`badge-${a.id}`} />}
      actions={(suppress) => (
        <MiniButton
          icon="trash"
          danger
...
function AgentEditor({ a, settings }: { a: AcpAgent; settings: SettingsStore }): ReactElement {
  ...
  return (
    <div>
      <FieldRow label="Label" modified={false}>
        <TextInput
      ...
      <FieldRow label="Command" modified={false}>
        <TextInput
      ...
      <FieldRow label="Args" modified={false}>
        <LinesField
